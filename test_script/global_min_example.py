...
if __name__ == '__main__':
    ...
    for FF in FF_lst:
        ...
        paras = model.model_parameters((t_list, eps_list, doping_para), lock=lock, k0=2*np.pi/0.98, surface_grating=True) # input tuple (t_list, eps_list, index where is the active layer)
        ...
        betas = np.linspace(2*np.pi/0.98*1, 2*np.pi/0.98*3.5, 10000)
