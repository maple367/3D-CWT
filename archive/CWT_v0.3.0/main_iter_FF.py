...
a = 0.295
beta_0 = 2*np.pi/a
FF_lst = np.linspace(0.05,0.40,36)
freq_lst = []
alpha_lst = []
...
order_arrary = np.array(FF_lst)
freq_arrary = np.array(freq_lst)[:,2:]
alpha_arrary = np.array(alpha_lst)[:,2:]

fig, ax = plt.subplots()
fig.set_dpi(300)
ax.plot(order_arrary, freq_arrary, '-o', label=['freq A', 'freq B',])
ax.set_xlabel('FF')
ax.set_ylabel('freq (c/a)')
ax.set_xlim(0.07, 0.24)
ax.set_ylim(0.290, 0.302)
ax2 = ax.twinx()
ax2.plot(order_arrary, alpha_arrary*1e4, '-*', label=['alpha A', 'alpha B'])
ax2.set_ylabel('alpha (1/cm)')
plt.legend(ax.get_legend_handles_labels()[0]+ax2.get_legend_handles_labels()[0], ax.get_legend_handles_labels()[1]+ax2.get_legend_handles_labels()[1])
plt.show()
