...
# 定义epsilon_0(x, y)
def epsilon_0(x, y):
    out_eps = np.ones((y.shape[0],x.shape[0]))
    eps_1 = np.ones_like(out_eps)
    XX, YY = np.meshgrid(x, y)
    out_eps = np.where((YY >= 1) & (YY < 1.1), 3.4824**2, out_eps)
    out_eps = np.where((YY >= 1.1) & (YY < 1.12), 3.46426**2, out_eps)
    out_eps = np.where((YY >= 1.12) & (YY < 1.33), 3.4461**2, out_eps)
    out_eps = np.where((YY >= 1.33) & (YY < 1.35), 3.46425**2, out_eps)
    out_eps = np.where((YY >= 1.35) & (YY < 1.43), 3.4824**2, out_eps)
    out_eps = np.where((YY >= 1.43) & (YY < 1.455), 3.2744**2, out_eps)
    out_eps = np.where((YY >= 1.455) & (YY < 1.475), 3.3799**2, out_eps)
    out_eps = np.where((YY >= 1.475) & (YY < 1.483), 3.5248**2, out_eps)
    out_eps = np.where((YY >= 1.483) & (YY < 1.503), 3.3799**2, out_eps)
    out_eps = np.where((YY >= 1.503) & (YY < 1.511), 3.5248**2, out_eps)
    out_eps = np.where((YY >= 1.511) & (YY < 1.571), 3.3799**2, out_eps)
    out_eps = np.where((YY >= 1.571) & (YY < 1.591), 3.3124**2, out_eps)
    out_eps = np.where((YY >= 1.591) & (YY < 2.641), 3.2449**2, out_eps)
    out_eps = np.where((YY >= 2.641) & (YY < 2.681), 3.36365**2, out_eps)
    out_eps = np.where(YY >= 2.681, 3.4824**2, out_eps)
    return out_eps
...
class Waveguide():
    def __init__(self):
        self.x = np.linspace(0, 0.297, 30)
        self.y = np.linspace(0.5, 3, 1250)

    def eps_func(self, x, y):
        return epsilon_0(x, y)

# %%
# import matplotlib.pyplot as plt
# plt.imshow(Waveguide().eps_func(Waveguide().x,Waveguide().y), cmap='Greys', interpolation='nearest', origin='lower', extent=[min(Waveguide().x), max(Waveguide().x), min(Waveguide().y), max(Waveguide().y)])
# plt.colorbar()
# plt.show()
...
def trapz2(f, x=None, y=None, dx=1.0, dy=1.0):
    """Double integrate."""
    return np.trapz(np.trapz(f, x=y, dx=dy), x=x, dx=dx)
...
class ModeSolverSemiVectorial():
    ...
    def build_matrix(self):
        ...
        eps = np.c_[eps[:, 0:1], eps, eps[:, -1:]]
        eps = np.r_[eps[0:1, :], eps, eps[-1:, :]]
        ...
        n = np.dot(ones_nx, 0.5 * (dy[:, 2:] + dy[:, 1:-1])).flatten()
        s = np.dot(ones_nx, 0.5 * (dy[:, 0:-2] + dy[:, 1:-1])).flatten()
        e = np.dot(0.5 * (dx[2:, :] + dx[1:-1, :]), ones_ny).flatten()
        w = np.dot(0.5 * (dx[0:-2, :] + dx[1:-1, :]), ones_ny).flatten()
        p = np.dot(dx[1:-1, :], ones_ny).flatten()
        q = np.dot(ones_nx, dy[:, 1:-1]).flatten()
        ...
        I = np.r_[iall, i_w, i_e, i_s, i_n]
        J = np.r_[iall, i_e, i_w, i_n, i_s]
        V = np.r_[Ap[iall], Ae[i_w], Aw[i_e], An[i_s], As[i_n]]
    ...
    def solve(self, neigs, tol=0, mode_profiles=True, initial_mode_guess=None):
        ...
        neff = self.wl * np.sqrt(eigvals) / (2 * np.pi)
        if mode_profiles:
            phi = []
...
class ModeSolverVectorial():
    ...
    def build_matrix(self):
        ...
        if isinstance(tmp, tuple):
            tmp = [np.c_[t[:, 0:1], t, t[:, -1:]] for t in tmp]
            tmp = [np.r_[t[0:1, :], t, t[-1:, :]] for t in tmp]
    ...
    def solve(self, neigs=4, tol=0, guess=None, mode_profiles=True, initial_mode_guess=None):
        ...
        neffs = self.wl * np.sqrt(eigvals) / (2 * np.pi)
        if mode_profiles:
            Hxs = []
            Hys = []
            nx = self.nx
...
class FDMode():
    ...
    def norm(self):
        x = centered1d(self.x)
        y = centered1d(self.y)
        return np.sqrt(trapz2(self.intensity(), x=x, y=y))
    ...
    def intensityTETM(self, x=None, y=None):
        I_TE = self.Ex * centered2d(np.conj(self.Hy)) / 2.
        I_TM = -self.Ey * centered2d(np.conj(self.Hx)) / 2.
...
# %%
def plot_mode(i, field_type='Ey'):
    import matplotlib.pyplot as plt
    import matplotlib.colors as mcolors
    fig = plt.figure(figsize=(10, 5))
    ax1 = fig.add_subplot(121)
    if field_type == 'Ex':
        field = sol.modes[i].Ex
    elif field_type == 'Ey':
        field = sol.modes[i].Ey
    elif field_type == 'Ez':
        field = sol.modes[i].Ez
    elif field_type == 'Hx':
        field = sol.modes[i].Hx
    elif field_type == 'Hy':
        field = sol.modes[i].Hy
    elif field_type == 'Hz':
        field = sol.modes[i].Hz
    
    field_r = np.real(field)
    divnorm = mcolors.TwoSlopeNorm(vmin=-np.abs(field_r).max(), vcenter=0, vmax=np.abs(field_r).max())
    ax1.imshow(field_r, cmap='bwr', norm=divnorm, origin='lower', extent=[min(Waveguide().x), max(Waveguide().x), min(Waveguide().y), max(Waveguide().y)])
    ax1.set_title(f'{field_type} neff={sol.modes[i].neff}')
    ax1.set_xlabel('x (um)')
    ax1.set_ylabel('y (um)')
    intensity = field_r ** 2
    intensity_projy = np.sum(intensity, axis=1) 
    intensity_projy_norm = intensity_projy / np.max(intensity_projy) * (max(Waveguide().x) - min(Waveguide().x)) + min(Waveguide().x)
    if field_type == 'Hx' or field_type == 'Hy' or field_type == 'Hz':
        ax1.plot(intensity_projy_norm, Waveguide().y, 'k--')
    elif field_type == 'Ex' or field_type == 'Ey' or field_type == 'Ez':
        ax1.plot(intensity_projy_norm, sol.xc, 'k--')
    ax1.set_aspect('auto')
    cbar1 = fig.colorbar(ax1.images[0], ax=ax1)

    ax2 = fig.add_subplot(122)
    ax2.imshow(Waveguide().eps_func(Waveguide().x,Waveguide().y), cmap='Greys', interpolation='nearest', origin='lower', extent=[min(Waveguide().x), max(Waveguide().x), min(Waveguide().y), max(Waveguide().y)])
    ax2.set_title('eps')
    ax2.set_xlabel('x (um)')
    ax2.set_ylabel('y (um)')
    ax2.set_aspect('auto')
    cbar2 = fig.colorbar(ax2.images[0], ax=ax2)
  
    plt.show()


# %%
# sol = ModeSolverVectorial(wl=0.980, structure=Waveguide(), boundary='00SS').solve(8)


# %%
# for i in range(8):
#     plot_mode(i, field_type='Ez')
